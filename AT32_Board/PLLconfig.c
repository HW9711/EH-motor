/**
  ...
  * Version: V1.x.x
  * Date   : 2020-xx-xx
  * Brief  : New PLL config program, Only for AT32F403A series
  ******************************************************************************
  */ 
...
static void StructInit(void)
{
  ...
  OscInitStruct.HSEPredivValue = RCC_HSE_DIV_2;
  ...
  OscInitStruct.PLLMUL = RCC_CFG_PLLMULT60;
  OscInitStruct.Over72MHz = RCC_Range_GreatThan_72Mhz;
...
static Status OscConfig(OscInitType *OscInitStruct)
{
  ...
  if(RCC_PLL_ENABLE == OscInitStruct->PLLState)
  {
    ...
    {
      if(RCC_CFG_PLLHSEPSC_HSE_DIV2 == OscInitStruct->PLLSourceHSEPsc)
      {
        RCC_HSEDivConfig(OscInitStruct->HSEPredivValue);
      }
      /* config hse divider */
    ...
    RCC->CFG |= (uint32_t)(OscInitStruct->PLLMUL | OscInitStruct->Over72MHz);
