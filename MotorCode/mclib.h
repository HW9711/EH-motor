...
#define   TWO_BY_SQRT3        (float)(1.1547005383f)   //Defines value for 2/sqrt(3)
...
#define   RhoOffsetMin         (M_PI / 32767.0f)
// *****************************************************************************
// *****************************************************************************
// Section: Data Types
// *****************************************************************************
...
typedef struct 
{
	...
	float   DCBusVoltage_T;   // Measured DC Bus voltage in volts
...
typedef struct 
{
    ...
    float   qErrDelta;
    float   qLastErr;
	float   qdSum_Max;	//PI输出最大值
	
		float		qdSumMax;	//积分项限幅
		float		qdSumMin;	//积分相限幅
...
{
    //----Com part
    float	  qDeltaT;      	//积分时间间隔						// Integration Interval
    float   qSquareDeltaT;		//积分时间间隔的平方
    float   qRho;    	    	//估计的转子角度					  // Estimated Rotor Angle
		
    float   qOmegaMr;     		//主速度估计值				// primary speed estimation
    float   qLastIalpha;  		//上一个Ialpha				// last value for Ialpha
    float   qLastIbeta;   		//上一个Ibeta				// last value for Ibeta
    float   qDIalpha;     		//Ialpha的差值				// difference Ialpha
    float   qDIbeta;      		//Ibeta的差值				// difference Ibeta
	  float 	qEsa;			//反电动势a轴				// BEMF alpha
	  float	  qEsb;				//反电动势β轴				// BEMF beta
	  float	  qEsd;				//反电动势d轴				// BEMF d
	  float	  qEsq;				//反电动势q轴				// BEMF q
    float	  qVIndalpha;   	//α轴的（电流*电感）/时间	电流变化乘以电感变化率	// dI*Ls/dt alpha
	  float	  qVIndbeta;    	//β轴的（电流*电感）/时间	电流变化乘以电感变化率					// dI*Ls/dt beta
	  float	  qEsdf;        	//反电动势d轴滤波值					// BEMF d filtered
	  float	  qEsqf;        	//反电动势q轴滤波值					// BEMF q filtered
	  float	  qKfilterEsdq; 	//d-q轴反电动势的滤波常数					// filter constant for d-q BEMF
	  float   qVelEstim; 		//转子的估计速度					// Estimated speed 
	  float   qVelEstimFilterK; //估计速度的滤波系数					// Filter Konstant for Estimated speed 
    float   qLastValpha;  		//上一次foc控制中的Ialpha				// Value from last control step Ialpha 
    float   qLastVbeta;   		//上一次foc控制中的Ibeta				// Value from last control step Ibeta
	  float   RhoOffset;       //估计的转子角度初始偏移量，单位是弧度     	// estimated rotor angle init offset in radians
    float	  qRs;				//电阻值，单位欧姆				// Rs value - stator resistance in ohms
	  float	  qLsDtq;			//电感随时间变化的值，与速度有关					// Ls/dt value - stator inductand / dt - variable with speed
    float   qLsDtd;				//定子电感对d轴的变化率
   	float	  qInvKFi;	    	//InvKfi常数值，（qInvKFi = 角速度/反电动势）				// InvKfi constant value ( InvKfi = Omega/BEMF )
    float   qKFi;              //反电动势常数，单位 V-秒/弧度                 // Backemf Constant in V-sec/rad
...
extern mcParam_PIController     			mcApp_Q_PIParam;     	 // Parameters for Q axis Current PI Controller 
extern mcParam_PIController     			mcApp_D_PIParam;     	 // Parameters for D axis Current PI Controller 
extern mcParam_PIController     			mcApp_Speed_PIParam; 	 // Parameters for Speed PI Controller 
extern mcParam_PIController					mcApp_VoltageP_PIParam;  //	Parameters for VoltageP
extern mcParam_PIController					mcApp_VoltageP_PIParam2;  //	Parameters for VoltageP

extern mcParam_FOC						      	mcApp_focParam;       // Parameters related to Field Oriented Control
extern mcParam_SinCos					      mcApp_SincosParam;    // Parameters related to Sine/Cosine calculator
extern mcParam_SVPWM 						    mcApp_SVGenParam;     // Parameters related to Space Vector PWM
extern mcParam_ControlRef 					  mcApp_ControlParam;   // Parameters related to Current and Speed references
extern mcParam_PLLEstimator          mcApp_EstimParam;     // Parameters related to PLL Estimator
extern mcParam_AlphaBeta             mcApp_I_AlphaBetaParam; // Alpha and Beta (2 Phase Stationary Frame) axis Current values
extern mcParam_DQ                    mcApp_I_DQParam;// D and Q axis (2 Phase Rotating Frame) current values
extern mcParam_ABC                   mcApp_I_ABCParam; // A,B,C axis (3 Phase Stationary Frame) current values
extern mcParam_AlphaBeta             mcApp_V_AlphaBetaParam; // Alpha and Beta (2 Phase Stationary Frame) axis voltage values
extern mcParam_DQ                    mcApp_V_DQParam;// D and Q axis (2 Phase Rotating Frame) voltage values
extern mcParam_PIController     		 mcApp_Cur_PIParam;


extern mcParam_PIController     			mcApp_Q2_PIParam;      // Parameters for Q axis Current PI Controller 
extern mcParam_PIController     			mcApp_D2_PIParam;      // Parameters for D axis Current PI Controller 
extern mcParam_PIController     			mcApp_Speed2_PIParam;  // Parameters for Speed PI Controller 
extern mcParam_PIController     			mcApp_Cur2_PIParam;

extern mcParam_FOC						      	mcApp_focParam2;       // Parameters related to Field Oriented Control
extern mcParam_SinCos					      mcApp_SincosParam2;    // Parameters related to Sine/Cosine calculator
extern mcParam_SVPWM 						    mcApp_SVGenParam2;     // Parameters related to Space Vector PWM
extern mcParam_ControlRef 					  mcApp_ControlParam2;   // Parameters related to Current and Speed references
extern mcParam_PLLEstimator          mcApp_EstimParam2;     // Parameters related to PLL Estimator
extern mcParam_AlphaBeta             mcApp_I2_AlphaBetaParam; // Alpha and Beta (2 Phase Stationary Frame) axis Current values
extern mcParam_DQ                    mcApp_I2_DQParam;// D and Q axis (2 Phase Rotating Frame) current values
extern mcParam_ABC                   mcApp_I2_ABCParam; // A,B,C axis (3 Phase Stationary Frame) current values
extern mcParam_AlphaBeta             mcApp_V2_AlphaBetaParam; // Alpha and Beta (2 Phase Stationary Frame) axis voltage values
extern mcParam_DQ                    mcApp_V2_DQParam;// D and Q axis (2 Phase Rotating Frame) voltage values
